...
async function initCalendar() {
  ...
  for (let i = 1; i <= lastDate; i++) {
    ...
    for (const eventObj of eventsArr) {
      if (
        ...
        if (eventObj.gardeJours > 0) {
          if(eventObj.gardeJours !== 999){
            persJ = eventObj.gardeJours === 999 ? null: await getPersonne(eventObj.gardeJours);
          }else{
            needGardeJours = true
          }
        }
    ...
    const initial = (persJ ? persJ.initial : "") + (pers?.initial ?? "")
    ...
    if (
      ...
      if (event) {
        days += `<div class="day today active event" data-initial="${initial}">${i}</div>`;
      } else {
        days += `<div class="day today active">${i}</div>`;
      }
    } else {
      if (event) {
        days += `<div class="day event ${needGardeJours ? "need": ""}" data-initial="${initial}">${i}</div>`;
      } else {
    ...
    if (
      ...
      if(need){
        garde += `<div class="event">
                <div class="title">
                  <i class="fas fa-circle"></i>
                  <h3 class="event-title">Besoin de quelqu'un</h3>
                </div>
                <div class="event-time">
                  <span class="event-time">Garde journée</span>
                </div>
...
async function getPersonne(idPersonne) {
  let pers = null;
  await fetch("./data/personne.json")
    .then((res) => res.json())
    .then((data) => {
